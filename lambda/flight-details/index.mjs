  ...
  try {
    ...
    if (!config.aerodatabox_enabled) {
      return {
        ...
        body: JSON.stringify({
          success: false,
          error: 'AeroDataBox API disabled by admin',
          disabled: true
        })
      };
    }
    ...
    if (!aircraftId) {
      return {
        ...
        body: JSON.stringify({
          success: false,
          error: 'aircraft_id (ICAO24) is required'
        })
      };
    }
    ...
    if (!flightDetails) {
      return {
        ...
        body: JSON.stringify({
          success: false,
          error: 'Flight not found or no active flight for this aircraft'
        })
      };
    }
...
async function fetchFlightDetails(icao24, apiKey) {
  try {
    ...
    // Format: /flights/search/Icao24/{icao24}?withAircraftImage=false&withLocation=false
    const url = `${API_MARKET_BASE_URL}/flights/search/Icao24/${icao24}?withAircraftImage=false&withLocation=false`;
